...
def restPost(appServer, user, password, file):
    ...
    content = f.read()
    f.close()
    url = 'https://' + appServer + '/phoenix/rest/incident/external?incident='
...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        print('Usage: updateIncidentAttributes.py appServer user password incidentAttributeDefFile')
        print('Example: python updateIncidentAttributes.py 192.168.20.116 super/admin adm1n incidentAttributeDef.json')
        sys.exit()
